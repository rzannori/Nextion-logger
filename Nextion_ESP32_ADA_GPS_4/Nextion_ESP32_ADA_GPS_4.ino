...
char folderNames[10][13]; // Array per memorizzare fino a 10 nomi di cartelle
...
void listFolders() {
  ...
  while (folderCount < 10) { // Limitato a 10 cartelle per evitare overflow di memoria
...
void sendFolderNamesToNextion() {
  // Calcola la dimensione necessaria per la stringa multilinea
  int bufferSize = 1;  // Inizia con spazio per il terminatore null
  for (int i = 0; i < folderCount; i++) {
    bufferSize += strlen(folderNames[i]);
    if (i < folderCount - 1) {
      bufferSize += 2;  // "\r\n"
    }
  }
  ...
  char* multilineData = (char*)malloc(bufferSize);
  if (multilineData == NULL) {
    return;
  }
  
  // Costruisci la stringa multilinea
  multilineData[0] = '\0';  // Inizializza come stringa vuota
  for (int i = 0; i < folderCount; i++) {
    strcat(multilineData, folderNames[i]);
    if (i < folderCount - 1) {
      strcat(multilineData, "\r\n");
    }
  }
  
  // Invia la stringa al Nextion
  myNex.writeStr("t0.txt", multilineData);
...
void trigger4() {
  // Ottiene il nome della cartella selezionata dal componente cb0 del Nextion
  String selectedValue = myNex.readStr("cb0.txt");
  
  // Verifica che il valore non sia vuoto
  if (selectedValue.length() > 0) {
    // Copia la stringa nel buffer
    strncpy(folderName, selectedValue.c_str(), 12);
    folderName[12] = '\0';  // Assicura che la stringa sia terminata correttamente
    
    // Formatta e invia data e ora al Nextion
    ...
    myNex.writeStr("t11.txt", formattedGGMM);
